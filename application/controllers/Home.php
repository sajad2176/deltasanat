...
class Home extends CI_Controller{
    ...
    public function index(){
        ...
        $buy_not = $this->base_model->run_query("SELECT SUM(d.volume) AS volume, max(h.volume_handle) AS handle  FROM  deltasanat.deal d LEFT JOIN (SELECT buy_id, SUM(volume_handle) AS volume_handle FROM deltasanat.handle GROUP BY buy_id) h ON h.buy_id = d.customer_id where d.type = 1 GROUP BY d.type" , 'row');
        if(empty($buy_not)){
            $data['buy_not'] = 0;
        }else{
            $data['buy_not'] = $buy_not->volume - $buy_not->handle;
        }
        $sell_not = $this->base_model->run_query("SELECT SUM(d.volume) AS volume, max(h.volume_handle) AS handle  FROM  deltasanat.deal d LEFT JOIN (SELECT sell_id, SUM(volume_handle) AS volume_handle FROM deltasanat.handle GROUP BY sell_id) h ON h.sell_id = d.customer_id where d.type = 2 GROUP BY d.type" , 'row');
        if(empty($sell_not)){
            $data['sell_not'] = 0;
        }else{
